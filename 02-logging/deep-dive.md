...
### zap 的 6 个真实痛点(为什么 2023 之后团队开始转 slog)

性能问题 zap 解决了，但日常用起来还有一堆小坑。下面 6 个都是中等规模团队真实会踩到的。
...
**更坑的是**：SugaredLogger 内部要把 `interface{}` 做 type switch 转成 `Field`，benchmark 上比 Logger 慢 2-3 倍，**接近 logrus 的水平**。新人一看 Sugared 和 slog 像，觉得方便就全用它，**白白放弃了 zap 的性能**。

#### 痛点 2 · error 日志默认丢堆栈和 wrap 链

```go
err := fmt.Errorf("get user: %w", sql.ErrNoRows)
logger.Error("login fail", zap.Error(err))
```

输出：
```json
{"level":"error","msg":"login fail","error":"get user: sql: no rows in result set"}
```

`error` 字段只是 `err.Error()` 字符串，**没有结构化的 wrap 链**。想看 `%+v` 级别的链路得自己手动：

```go
logger.Error("login fail",
    zap.Error(err),
    zap.String("err_chain", fmt.Sprintf("%+v", err)),
    zap.Stack("stack"),
)
```
...
zap 没有 `logger.InfoContext(ctx, ...)` 方法。想把 ctx 里的 trace_id 带进日志，每个团队都要自己写 helper：
...
func logWithTrace(ctx context.Context) *zap.Logger {
    return zap.L().With(zap.String("trace_id", traceIDFromCtx(ctx)))
}
```

没有官方 API，**5 个微服务 5 种写法**。

#### 痛点 4 · 给所有日志加一个字段，要改 N 处

K8s 部署想让日志都带 `pod_name`。zap 标准做法：

```go
pod := os.Getenv("HOSTNAME")
logger = logger.With(zap.String("pod", pod))   // ← 这行要出现在每个入口
```

HTTP 入口、gRPC 入口、Cron、Worker、Consumer……**每个入口都要记得加**。新人加一个 worker，十有八九漏。审计时得 grep `zap.New` 逐个检查。
...
这 6 个痛点，logrus 也有一半（性能除外）。**slog 设计的时候，直接把这 6 个都当第一优先级解决了**。
...
### 场景 1 · 给所有日志加 `pod_name`，不改一行业务代码

**痛点**：K8s 部署要求所有日志带 `pod_name`。业务代码散在 20 个入口（HTTP、gRPC、Cron、Worker、Consumer），每个入口都改一次？漏一个就没字段。
...
// 启动时
pod := os.Getenv("HOSTNAME")
rootLogger = rootLogger.With(zap.String("pod", pod))

// 然后每个入口都要记得传这个 logger：
func StartHTTPServer(logger *zap.Logger) { ... }
func StartGRPCServer(logger *zap.Logger) { ... }
func StartWorker(logger *zap.Logger)     { ... }
// 每加一个入口都要记得；漏一个 code review 肉眼查
```

**slog 方案**：写一个 Handler 包装器，**一次生效，业务代码零修改**：
...
func (h *PodHandler) Handle(ctx context.Context, r slog.Record) error {
    r.AddAttrs(slog.String("pod", h.pod))
    return h.Handler.Handle(ctx, r)
}

func (h *PodHandler) WithAttrs(attrs []slog.Attr) slog.Handler {
    return &PodHandler{h.Handler.WithAttrs(attrs), h.pod}
}
func (h *PodHandler) WithGroup(name string) slog.Handler {
    return &PodHandler{h.Handler.WithGroup(name), h.pod}
}

// main.go 一次配置
slog.SetDefault(slog.New(&PodHandler{
    Handler: slog.NewJSONHandler(os.Stdout, nil),
    pod:     os.Getenv("HOSTNAME"),
}))
```

之后所有地方 `slog.Info(...)` 都自动带 `pod` 字段。加新入口？不需要做任何事。

> **这就是解耦的第一个收益：把"横切关注点"从业务代码里剥离出来。**

---

### 场景 2 · error 日志自动展开完整 wrap 链

**痛点**（对应 zap 痛点 2）：`logger.Error("fail", zap.Error(err))` 只打 `err.Error()`，丢了 `%w` wrap 的每一层。想看完整链路，每处都要额外加 `zap.String("err_chain", fmt.Sprintf("%+v", err))`，规范靠自觉。

**slog 方案**：Handler 层统一兜底：

```go
type ErrChainHandler struct {
    slog.Handler
}

func (h *ErrChainHandler) Handle(ctx context.Context, r slog.Record) error {
    r.Attrs(func(a slog.Attr) bool {
        if err, ok := a.Value.Any().(error); ok {
            r.AddAttrs(slog.String(a.Key+"_chain", fmt.Sprintf("%+v", err)))
        }
        return true
    })
    return h.Handler.Handle(ctx, r)
...
业务代码**照常写** `slog.Error("db fail", "err", err)`，日志里自动多一个 `err_chain` 字段。新人忘了也没事，Handler 兜住。

---

### 场景 3 · Prod 想动态打开 DEBUG 10 分钟排障

**痛点**：线上默认 INFO，出事了想临时开 DEBUG 排查，不想重启。

**zap** 有 `zap.AtomicLevel`，但要自己接 HTTP 接口，且 AtomicLevel 本身不在 zapcore 里，要从 zap 包引入。
...
一行 `curl /admin/log-level?level=debug` 立即生效，**全链路所有 Handler 共享这个原子变量**。
...
### 场景 6 · 单元测试断言 "是否打了某条日志"

**痛点**：业务要求某分支必须打 error，想写单测保证不会悄悄不打。

**zap 方案** —— 用 `zaptest/observer`：
...
能用，但 `observer.ObservedLogs` 是 zap 专属类型，单测多一份认知负担。

**slog 方案** —— 自己写一个 20 行的 Handler，**它和生产代码用的是同一份 `slog.Handler` 接口**：
...
**统一的 Handler 接口意味着测试的 logger 和生产的 logger 是同一套抽象**，不用学两套 API。
...
### 场景 8 · 本地看人读格式，线上看机读 JSON

**痛点**：本地开发想看彩色 + key=value 文本；线上要 JSON 给 ELK。

**slog 方案**：同一份业务代码，**Handler 按环境换**：
...
业务代码就是 `slog.Info("user login", "uid", 123)`，**输出格式在入口一行切换**。
想要更好看的彩色 console？用 `phsym/console-slog`，还是一个 `slog.Handler`。

zap 也能做，但 `zap.NewDevelopment()` / `zap.NewProduction()` 是两套预设配置，自定义多一项都要重写 config。
...
| 场景 | 主要靠哪个设计 |
|------|---------------|
| 1 加 pod_name | **Handler 解耦** + WithAttrs 预编码 |
| 2 展开 err chain | **Handler 解耦**（拦截 Record） |
| 3 动态 level | **Handler 可插拔** + `LevelVar` |
| 4 敏感字段 | **LogValuer** + ReplaceAttr |
| 5 接 Sentry/Loki | **Handler 解耦** + 统一生态接口 |
| 6 单元测试 | **Handler 解耦**（Handler = 可测试单元）|
| 7 库不绑后端 | **零依赖**（标准库） |
| 8 dev vs prod | **Handler 解耦** + 内置 Text/JSON Handler |

你可能注意到，**"Handler 解耦"出现了 6 次**。不是巧合 —— 它是 slog 所有灵活性的根源。LogValuer 和零依赖则各自解决一个具体问题（脱敏/懒求值、依赖传染）。
