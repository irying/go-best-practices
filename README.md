...
| 02 | [日志](./02-logging/) | [slides.md](./02-logging/slides.md) · [layering-slides.md](./02-logging/layering-slides.md) · [deep-dive-slides.md](./02-logging/deep-dive-slides.md) · [deep-dive.md](./02-logging/deep-dive.md) | [slides.pptx](./02-logging/slides.pptx) · [layering-slides.pptx](./02-logging/layering-slides.pptx) · [deep-dive-slides.pptx](./02-logging/deep-dive-slides.pptx) | 新项目默认 `log/slog`；按读者分四层；应用只写 stdout，收集交给 agent |
...
| 14 | **库选型:stdlib · x/ext · 第三方** | Go 团队对 "推荐库" 的克制态度；`slices`/`maps`/`iter` 替代 `samber/lo`；`x/sync`/`x/time` 是 "准 stdlib"；`google/go-cmp`/`uber-go/goleak` 事实标准；哪些流行库(testify/viper)要按场景权衡 |
